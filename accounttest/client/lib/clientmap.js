  ...
  this._map = new this._ol.Map(
    {
      ...
      view: new ol.View({ // setup view, if done later the map will not show
...
ClientMap.prototype.defaultSettings = function() {
  ...
  var layer = new this._ol.layer.Tile(
    { 
      source: new this._ol.source.MapQuest({layer: 'sat'})
    });
...
ClientMap.prototype.createVectorLayer = function(name){
  ...
  vector = new this._ol.layer.Vector({ // create a named layer
    ...
    isBaseLayer: false // regared as the baselayer
  });
...
ClientMap.prototype.removeFeature = function(layername, feature){
	this._map.getLayers().forEach(function(layer) {  //iterate layers
	    if (layer.get('name') == layername) {  // if layer is the 'one'      
	    	...
	    	layer.getSource().removeFeature(feature.get('name')); // add feature to the layer
	    } 
	  });
};
...
// Transforms an incomming class --e.g. from mongodb-- to an 
// OpenLayers Feature.
...
ClientMap.prototype.featureTransform = function(item) {
  ...
  return new this._ol.Feature({ // transform to Feature
    geometry: new this._ol.geom.Point(point),
    labelPoint: new this._ol.geom.Point(point),
    name: item._id // store the mongodb id as name.
  });
};
